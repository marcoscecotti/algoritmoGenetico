...
def evaluarFitness(nro_poblacion,fenotipo,funcion):
    fitness = np.zeros(nro_poblacion)
    for i in range(nro_poblacion):
        fitness[i] = funcion(float(fenotipo[i]))
...
def reemplazoPoblacional(nro_poblacion,hijos,idx_mejor_fitness,genotipo):
    ...
    nuevaGeneracion = hijos
    eliminarHijo = random.randint(0, nro_poblacion - 1)
    nuevaGeneracion[eliminarHijo] = genotipo[idx_mejor_fitness]
    return nuevaGeneracion



def algoritmoGenetico(nro_poblacion, nro_bits_individuo, nro_seleccion, nro_competencia, prob_cruza, prob_mutacion,
                      it_max, min_rango, max_rango, funcion, metodo_seleccion, fitness_buscado, puntoInicialCruzayMuta):

    # Inicializar problación
    genotipo, fenotipo = inicializarPoblacion(nro_poblacion,nro_bits_individuo,min_rango,max_rango)
    # print(fenotipo)
    # print(genotipo)

    # Evaluar función de Fitness
    fitness, mejor_fitness, idx_mejor_fitness = evaluarFitness(nro_poblacion, fenotipo, funcion)
    #print(fitness)
    ...
    while it < it_max and mejor_fitness < fitness_buscado:
        ...
        # Reproducción
        hijos = reproduccion(progenitores, genotipo, nro_bits_individuo, nro_poblacion, prob_cruza, prob_mutacion,
                             min_rango, max_rango, puntoInicialCruzayMuta)

        # Reemplazo poblacional (Elitismo)
        nuevaGeneracion = reemplazoPoblacional(nro_poblacion, hijos, idx_mejor_fitness, genotipo)
        genotipo = nuevaGeneracion

        # Evaluar población
        fenotipo = decodificar(nro_poblacion, genotipo)
        fitness, mejor_fitness, idx_mejor_fitness = evaluarFitness(nro_poblacion, fenotipo, funcion)

        it = it + 1

    return mejor_fitness, fenotipo[idx_mejor_fitness]